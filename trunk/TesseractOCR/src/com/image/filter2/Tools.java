...
public class Tools {
    ...
    public static BufferedImage[] getCheckCodes(BufferedImage image) {
        ...
        checkCode[0] = image.getSubimage(0 * (width / checkCode.length), 0,21
                                                                            ,
            height);
        checkCode[1] = image.getSubimage(1 * (width / checkCode.length) + 3, 0,10, height);
        checkCode[2] = image.getSubimage(2 * (width / checkCode.length), 0, 14,
            height);
        checkCode[3] = image.getSubimage(3 * (width / checkCode.length) , 0, width-21-10-14,
            height);
